...
require 'demisus/view/html_rule_debugger'
require 'demisus/importers'

def die_in_error(extra_msg=nil)
  $stderr.puts "ERROR: #{extra_msg}" if extra_msg
  $stderr.puts "Syntax: #{$0} <path/to/sudoku/file> <rule_id>"
  exit 1
end

sudoku_path, rule = ARGV[0], ARGV[1]

if sudoku_path.nil? or rule.nil?
...
exporter = Demisus::View::HtmlRuleDebugger.new(solver, rule.to_sym)
